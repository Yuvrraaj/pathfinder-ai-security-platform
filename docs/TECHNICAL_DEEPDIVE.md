...
**Why "JSON only"**: Prevents Claude from adding explanatory text that breaks parsing.
...
**Why JSON schema in prompt**: Shows Claude exactly what structure to produce. Reduces parsing errors.
...
**Why strip markdown**: Claude sometimes wraps JSON in code blocks despite instructions.
