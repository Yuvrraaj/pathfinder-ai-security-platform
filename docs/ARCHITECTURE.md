...
A prompt-engineered Claude integration with structured output:
...
1. **Structured output**: Claude follows JSON schemas reliably
2. **Reasoning quality**: Better at security domain reasoning than smaller models
3. **Anthropic relationship**: RedHold likely has Anthropic integration already

### Why I Included a Fallback

Not everyone has `ANTHROPIC_API_KEY` set. For demos and testing, I built a **heuristic fallback** that:
